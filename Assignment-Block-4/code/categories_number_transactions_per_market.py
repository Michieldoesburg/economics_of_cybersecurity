...
for market_index in range(len(unique_markets)):
	filename = "%s.csv" % (unique_markets[market_index])
	with open(filename, 'w') as csvfile:
		...
		#Get all data
		for year in yearly_overview:
			if str(year) != "2017" and str(year) != "2018":
				overview = yearly_overview[year]
