...
markets = data_feedback["marketplace"].tolist()
...
categories = data_feedback["category"].tolist()
del vendor_hashes[0]
del markets[0]
del item_hashes[0]
del total_sales[0]
...
def get_most_popular_category_per_market(item_sales, item_hashes, categories, markets, unique_markets, unique_categories):
	market_categories = dict()
	for market in unique_markets:	
		market_categories[market] = []
		for category in unique_categories:
			market_categories[market].append(0)
	
	for item_hash in item_sales:
		index = item_hashes.index(item_hash)
		category = categories[index]
		market = markets[index]
		market_categories[market][unique_categories.index(category)] = market_categories[market][unique_categories.index(category)] + 1

	for market in unique_markets:
		category_index = market_categories[market].index(max(market_categories[market]))
		print(market)
		print(market_categories[market])
		market_categories[market] = unique_categories[category_index]
...
for year in years:
	...
	popular_categories = get_most_popular_category_per_market(item_sales, feedback_item_hashes, categories, markets, unique_markets, unique_categories)

	for market in popular_categories:
		print(market)
		print(popular_categories[market])
