...
for index in range(len(feedback_item_hashes)):
	if feedback_item_hashes[index] in item_sales and str(feedback_item_date[index]) == str(g_year):
		item_sales[feedback_item_hashes[index]] = item_sales[feedback_item_hashes[index]] + float(feedback_item_value[index])
	elif str(feedback_item_date[index]) == str(g_year):
		item_sales[feedback_item_hashes[index]] = 0
...
for item_number in range(len(item_hashes)):
	if item_hashes[item_number] in item_sales:
		...
		vendor_revenue[vendor_hashes[item_number]] = float(vendor_revenue[vendor_hashes[item_number]]) + float(item_sales[item_hashes[item_number]])
print("Finished creating vendor-item combinations.")

#Calculate revenue per market in a specific year
...
for index in range(len(revenue_accumulated_markets)):
	...
	for revenue in market_revenue:
		if revenue_per_market[index] == 0:
			market_revenue_percentage.append(0)
		else:
...
plt.title('Percentage of vendors representing percentage of total revenue per market in %d' % g_year)
